...
public class ProductosRepository implements IProductosRepository {
	...
	public List<Producto> lista() {
		SimpleJdbcCall jdbcCall = new SimpleJdbcCall(jdbc).withProcedureName("ObtenerProductos")
	...
	public int guardar(Producto producto) {
		SimpleJdbcCall jdbcCall = new SimpleJdbcCall(jdbc).withProcedureName("InsertarProductos");
	...
	public int actualizar(Producto producto) {
		String sqlConsulta = "UPDATE Productos SET CodigoBara = ?, Nombre =?, Marca=?, Categoria=?, Precios=?"
				+ "WHERE IdProducto =? ";
		return jdbc.update(sqlConsulta, new Object[] { producto.getCodigoBara(), producto.getNombre(),
				producto.getMarca(), producto.getCategoria(), producto.getPrecios(), producto.getIdProducto() });
	}
